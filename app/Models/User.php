...
class User extends Authenticatable {
  ...
  public function getIdImageAttribute() {
    if ($this->attributes['id_image']) {
      $image = $this->getImage($this->attributes['id_image'], 'users');
    } else {
      $image = $this->defaultImage('users');
    }
    return $image;
  ...
  public function setIdImageAttribute($value) {
    if (null != $value && is_file($value)) {
      isset($this->attributes['id_image']) ? $this->deleteFile($this->attributes['id_image'], 'users') : '';
      $this->attributes['id_image'] = $this->uploadAllTyps($value, 'users');
    }
  }
  ...
  public function transferTransactions() {
    return $this->hasMany(transferTransaction::class, 'sender_id')->latest();
  }

  public function transferAmounts() {
    return $this->hasMany(transferAmount::class, 'sender_id')->latest();
  }
